...
async def guardar_acta(request: Request):
    ...
    for clave in datos:
        ...
        nombre = datos.get(f"{prefijo}_nombre_{indice}")
    ...
    with open(
        ...
        json.dump(debates, f, ensure_ascii=False, indent=4)
...
async def exportar_datos():
    path = os.path.join(BASE_DIR, "datos", "debates.json")
    return FileResponse(
        path,
        media_type="application/json",
        filename="debates.json"
    )
...
async def borrar_debate(id: str):
    ...
    debates = [d for d in debates if d.get("id") != id]
...
async def clasificacion_datos():
    ...
    for acta in debates:
        clave = (acta.get("ronda"), acta.get("sala"))
    ...
    resultado = sorted(
        ...
        key=lambda x: (-x["pg"], -x["pts"], x["faltas"])
    )
...
async def clasificacion_oradores():
    ...
    for debate in debates:
        for orador in debate.get("oradores", []):
            ...
            ranking[nombre]["puntos"] += float(orador["nota"])
...
async def editar_debate(request: Request, id: str):
    ...
    if not acta:
        return HTMLResponse("Acta no encontrada", status_code=404)
...
async def actualizar_debate(request: Request, id: str):
